...
from .serializers import CREATEMaritalStatusSerializer, UPDATEMaritalStatusSerializer, GETMaritalStatusByIdSerializer, GETAllMaritalStatusSerializer, GETAllMaritalStatusForDropDownSerializer
from rest_framework.response import Response
from rest_framework.permissions import IsAuthenticated
from utils.get_id_by_token import get_user_id_from_token_view
from .models import MstMaritalStatus
import logging
...
class GETAllMaritalStatus(APIView):

    def get(self, request, *args, **kwargs):
        try:
            query_set = MstMaritalStatus.objects.all().order_by('order')
            if len(query_set) < 1:
                response_data = {
                    'status': 200,
                    'statusCode': 'success',
                ...
                return Response(response_data)
            serializer = GETAllMaritalStatusSerializer(query_set, many=True)
            response_data = {
                'status': 200,
                'statusCode': 'success',
                'data': serializer.data

...
from masterApi.models import MstSect, MstBloodGroup, MstMaritalStatus, MstRelation, MstProfession
...
class HeadSerializer(BaseSerializer):
    ...
    cityId = serializers.PrimaryKeyRelatedField(queryset=City.objects.all(), required=True)
    areaId = serializers.PrimaryKeyRelatedField(queryset=Area.objects.all(), required=True)
    sectId = serializers.PrimaryKeyRelatedField(queryset=MstSect.objects.all(), required=True)
...
class MemberSerializer(BaseSerializer):
    headId = serializers.PrimaryKeyRelatedField(queryset=User.objects.filter(headId=None), required=True)
    dob = serializers.DateField(input_formats=("%B %d, %Y",), required=False)
    relationId = serializers.PrimaryKeyRelatedField(queryset=MstRelation.objects.all(), required=True)
...
class UpdateMemberSerializer(BaseSerializer):
    dob = serializers.DateField(input_formats=("%d %B, %Y",), allow_null=True)
...
class GETFamilyByHeadIdSerializer(serializers.ModelSerializer):
    ...
    relation = serializers.CharField(source='relationId.description', default="")
...
class GETAllMemberByAreaIdSerializer(serializers.ModelSerializer):
    ...
    def get_phoneNumber(self, instance):
        print('phoneNumber==> ', instance.phoneNumber)
        is_number_visible = instance.phoneNumberVisibility
        if not is_number_visible:
            return 'xx-xxxx-xxxx'
        return str(instance.phoneNumber)
